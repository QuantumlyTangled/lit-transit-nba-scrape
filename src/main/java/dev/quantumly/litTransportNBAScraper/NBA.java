...
public class NBA implements Callable<Integer> {
   ...
   * @return The final URL to use for scraping a profile page after a potential redirect has been
   * executed
   */
  private static String getRedirectableSearch(String profile) throws IOException {
    Connection initialPlayerProfile = Jsoup
      .connect("https://www.basketball-reference.com/search/search.fcgi?search=" + profile);
    ...
    if (followedUrl.url().toString().contains("?")) {
      Document searchDocument = Jsoup
        .connect("https://www.basketball-reference.com/search/search.fcgi?search=" + profile).get();
      return "https://www.basketball-reference.com" + searchDocument.select(".search-item-name")
        .first().select("a").first().attr("href");
    } else
      return followedUrl.url().toString();
  }
  ...
  private static Object[] extractTableValues(Document document, String stat) {
    ...
    var RawFiltered = Arrays.stream(Raw)
      .filter(e -> !(e.toString().contains("season") || e.toString().contains("Career"))).toArray();
